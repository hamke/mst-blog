...
.then(wptalk_themes_free_result => {
  ...
  if ( wptalk_themes_free_result !== null && wptalk_themes_free_result !== '') {
    ...
    if ( output_themes_free !== '' ) {
      document.getElementById('wpsalenow-themes-free').innerHTML = '<h2>◼︎ WordPress Theme Real-Time Promotions (Free)</h2>' + output_themes_free + '<hr>';
    }
  }
})
...
.then(wptalk_themes_sale_result => {
  ...
  if ( wptalk_themes_sale_result !== null && wptalk_themes_sale_result !== '') {
    ...
    if ( output_themes_sale !== '' ) {
      document.getElementById('wpsalenow-themes-sale').innerHTML = '<h2>◼︎ WordPress Theme Real-Time Promotions (Discount)</h2>' + output_themes_sale + '<hr>';
    }
  }
})
...
.then(wptalk_plugins_sale_result => {
  ...
  if ( wptalk_plugins_sale_result !== null && wptalk_plugins_sale_result !== '') {
    ...
    if ( output_plugins_sale !== '' ) {
      document.getElementById('wpsalenow-plugins-sale').innerHTML = '<h2>◼︎ WordPress Plugin Real-Time Promotions (Discount)</h2>' + output_plugins_sale + '<hr>';
    }
  }
})
...
.then(wptalk_hosting_sale_result => {
  ...
  if ( wptalk_hosting_sale_result !== null && wptalk_hosting_sale_result !== '') {
    ...
    if ( output_hosting_sale !== '' ) {
      document.getElementById('wpsalenow-hosting-sale').innerHTML = '<h2>◼︎ WordPress Hosting Real-Time Promotions (Discount)</h2>' + output_hosting_sale + '<hr>';
    }
  }
})
